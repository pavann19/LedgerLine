...
PROJECTION_API_URL="${PROJECTION_URL}/api/v1"
...
ACC_A=$(curl -sf -X POST "${API_URL}/accounts" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"currency":"USD","type":"CUSTOMER"}' | grep -o '"id":"[^"]*' | cut -d'"' -f4)

ACC_B=$(curl -sf -X POST "${API_URL}/accounts" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"currency":"USD","type":"CUSTOMER"}' | grep -o '"id":"[^"]*' | cut -d'"' -f4)
...
ACC_FUNDING=$(curl -sf -X POST "${API_URL}/accounts" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
...
curl -sf -X POST "${API_URL}/transfers" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
...
TX_RES=$(curl -sf -X POST "${API_URL}/transfers" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
...
REPLAY_RES=$(curl -sf -X POST "${API_URL}/transfers" -H "Authorization: Bearer ${ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: ${IDEMP_KEY_TX}" \
  -d "{\"fromAccountId\":\"${ACC_A}\",\"toAccountId\":\"${ACC_B}\",\"amountMinor\":12500,\"currency\":\"USD\"}")
...
BAL_A=$(curl -sf -H "Authorization: Bearer ${ACCESS_TOKEN}" "${API_URL}/accounts/${ACC_A}" | grep -o '"balanceMinor":[0-9]*' | cut -d':' -f2)
BAL_B=$(curl -sf -H "Authorization: Bearer ${ACCESS_TOKEN}" "${API_URL}/accounts/${ACC_B}" | grep -o '"balanceMinor":[0-9]*' | cut -d':' -f2)
...
curl -sf -H "Authorization: Bearer ${ACCESS_TOKEN}" "${API_URL}/accounts/${ACC_A}/postings"
echo ""
